...
export default async function VisualResumePage() {
    ...
    <>
      <style
        dangerouslySetInnerHTML={{
          __html: `
/*
 * The designed résumé. Its plain twin at /resume is the one built to survive
 * a parser; this one is built to be looked at, and carries the site's own
 * palette, face and window chrome onto paper.
 *
 * Every colour here is a background that has to survive printing, so the
 * sheet asks for exact colour rather than letting a printer drop it — a dark
 * document that prints white is not a document.
 */
...
.vr {
   ...
   */
  background:
    radial-gradient(58% 32% at 14% 0%, rgba(120, 210, 180, 0.22), transparent 72%),
    radial-gradient(54% 30% at 78% 1%, rgba(150, 140, 255, 0.2), transparent 74%),
    radial-gradient(46% 24% at 46% 0%, rgba(90, 180, 220, 0.15), transparent 70%),
    var(--bg);
  color: var(--ink);
                      ...
                      {group.items.map((skill) => (
                        <span key={skill.id} className="vr-tag">
                          {skill.logoUrl ? (
                            ...
                            <span className="vr-mark vr-mark-fallback">
                              {skill.name.charAt(0)}
                            </span>
                          )}
                          {skill.name}
              ...
              <section>
                ...
                {projects.map((project) => (
                  <div key={project.id} className="vr-work">
                    ...
                    {project.summary ? (
                      <div className="vr-work-note">{project.summary}</div>
                    ) : null}
                  </div>
                ))}
              </section>
